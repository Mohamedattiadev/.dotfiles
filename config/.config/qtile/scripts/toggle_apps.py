...
anki_group = "S"  # You can choose a group for Anki
qute_group = "5"  # You can choose a group for Anki
anki_class = "Anki"
file_manager_class = "pcmanfm"
qute_class = "qutebrowser"
brave_group = "2"  # You can choose a group for Anki
brave_class = "brave-browser"
google_chrome_class = "google-chrome"
alacritty_class = "alacritty"
alacritty_group = "4"  # Target group
...
def toggle_alacritty(qtile):
    ...
    if current_win:
        wm_class = current_win.get_wm_class()
        if wm_class and any(alacritty_class.lower() in c.lower() for c in wm_class):
            is_alacritty = True

    # Already on Alacritty in group 4 → go back
    if current_group == alacritty_group and is_alacritty:
        if last_group[0]:
            qtile.groups_map[last_group[0]].toscreen()
        return
    ...
    target_win = None
    for w in qtile.groups_map[alacritty_group].windows:
        wm_class = w.get_wm_class()
        if wm_class and any(alacritty_class.lower() in c.lower() for c in wm_class):
            target_win = w
            break
    ...
    if target_win:
        last_group[0] = current_group
        _focus_window_and_group(qtile, alacritty_group, target_win)
        return

    # Not open in group 4 → spawn it
    last_group[0] = current_group
    qtile.groups_map[alacritty_group].toscreen()
